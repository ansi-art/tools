(function (root, factory){
    if(typeof define === 'function' && define.amd){
        ...
        define(['maplex', '@ansi-art/color'], factory);
    }else if (typeof module === 'object' && module.exports){
        module.exports = factory(require('maplex'), require('@ansi-art/color'));
    }else{
        // Browser globals (root is window)
        if(!root.Ascii) root.Ascii = {};
        if(!root.Ascii.AnsiTools) root.Ascii.AnsiTools = {};
        root.Ascii.AnsiTools.Ansi = factory(root.Maplex, root.AsciiArtColor);
    }
}(this, function(Maplex, AnsiArtColor){
    var AsciiArt = {};
    var parentArt;
     ...
     */
    AsciiArt.Ansi = function(opts){
        const options = opts || {};
        if(!options.bitDepth) options.bitDepth = '4bit'
        let palette = new AnsiArtColor.Palette({
            medium: new AnsiArtColor.Medium(options.medium || 'vga'),
            space: new AnsiArtColor.Space(options.bitDepth),
                ...
                for(var lineNumber=0; lineNumber < lines.length; lineNumber++){
                    ...
                    for(; lcv - offset < line.length; lcv++){
                        ...
                        if(inEscape){
                            ...
                            if(line[lcv- offset] == '\0'+'33' && line[lcv- offset+1] == '['){
                                inEscape = true;
                                code = '';
                                lcv++;
                                continue;
